...
<select name="fund_id">
  <option ';
	...
	echo ' >STF</option>
  <option '; if ($_REQUEST["fund_id"]=="TF") { echo 'selected="selected"';}
        echo ' >TF</option>
  <option '; if ($_REQUEST["fund_id"]=="IGP") { ECHO 'selected="selected"';}
	echo '>IGP</option>
  <option '; if ($_REQUEST["fund_id"]=="GF") { ECHO 'selected="selected"';}
	echo '>GF</option>
</select>
	 </td></tr>';
	...
	if( isset($_REQUEST["view"]) ) {
		...
		$list->connect();
		$list->get_list_of_feeelement_new($sy_id,$_REQUEST["fund_id"],$_REQUEST["collector_id"]);
		for( $i=0; $i<$list->get_numrows(); $i++ ) {
			$dat = $list->get_fetch_assoc($i);
			$feeelement_array[$dat["feeelement_id"]] = $dat["title"];
		}
		$result = $list->get_list_by_feeelement_new($sy_id_from,$sy_id_to,$date_from,$date_to,$_REQUEST["fund_id"],$_REQUEST["collector_id"]);
		if( $result != false ) {
			set_time_limit( 90 );
			echo "<table border=\"1\" cellspacing=\"0\" cellpadding=\"0\" style=\"font-size:small\">";
			echo "<tr>";
			echo "<th>Date</th><th>ORNO</th><th>StudentID</th><th> StudentName </th><th>Department</th><th>Total</th>";
			...
			for( $i=0; $i<$list->get_numrows()+1; $i++ ) {
				...
				if( ($id!=$this_id || $i==$list->get_numrows()) && ($id!='') ) {
					...
					foreach( $feeelement_array as $feeelement_id=>$title ) {
						if( $dat[$feeelement_id]==0 ) {
							...
							echo '<td align="center">' . $dat[$feeelement_id] . ' </td>';
						} else {
				...
				if( $id!=$this_id ) {
					...
					$dat["dep"]=$this_dat["dep"];
				}
				$dat[$this_dat["feeelement_id"]] = $this_dat["payment"];
				$dat["total"] += $this_dat["payment"];
			}
			echo '<tr>';
			echo '<td align="right">Total</td><th>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>';
