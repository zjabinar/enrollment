...
<title> List of guarantors  </title>
</head>
...
<?php
	...
	print_title( get_office_name($_SESSION["office"]), "List of guarantors", $str_schoolyear );

	echo '<form method="POST" name="mainform">';

	$result = false;
	$list = new model_guarantor;
	$list->connect();
	...
	if( $result != false ) {
		...
		echo "<tr>";
		echo "<th>&nbsp;</th>";
		echo "<th>StudentID</th><th>Name</th><th>Course</th><th>Guarantor</th><th>Due date</th><th>Remark</th></tr>";
		echo "</tr>";
		for( $i=0; $i<$list->get_numrows(); $i++ ) {
			...
			printf( "<td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td> <td>%s</td>\n",
				...
				lookup_teacher_name($dat["teacher_id"]),
				mkstr_neat( mkstr_date( $dat["due_date"] ) ),
				mkstr_neat( $dat["remark"] )
			);
			printf( "</tr>" );
		}
		echo '</table>';
