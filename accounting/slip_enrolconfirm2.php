...
<?php
	...
	*/
	$output .= '<div style="position:absolute; top:1.7cm; left:0cm; width:' . $size_x . '; text-align:right">' . date("M j, Y") . '&nbsp;</div>';
	$output .= '<div style="position:absolute; top:2.4cm; left:0.4cm"><strong>' . mkstr_name_lfm($dat["first_name"],$dat["middle_name"],$dat["last_name"]) . '</strong></div>';
	$output .= '<div style="position:absolute; top:2.4cm; left:9.0cm"><strong>' . mkstr_student_id($student_id) . '</strong></div>';
	//$output .= '<div style="position:absolute; top:3.2cm; left:0cm; width:' . $size_x . ';">' . lookup_schoolyear($sy_id). ' semester</div>';
	$sem_evsu = evsu_sem($sy_id);
	if ($sem_evsu=="1st") {
		$output .= '<div style="position:absolute; top:3.2cm; left:4.0cm">x</div>';
	}elseif ($sem_evsu=="2nd") {
		$output .= '<div style="position:absolute; top:3.2cm; left:5.0cm">x</div>';
	}elseif ($sem_evsu=="3rd") {
		$output .= '<div style="position:absolute; top:3.2cm; left:6.2cm">x</div>';
	}

	$output .= '<div style="position:absolute; top:3.2cm; left:0.4cm; width:' . $size_x . ';">' . evsu_sy($sy_id). '</div>';
	$output .= '<div style="position:absolute; top:3.3cm; left:8.2cm"><strong>' . get_short_course_from_course_id($dat["course_id"]) . ' (' .  lookup_yearlevel($dat["year_level"]) . '-' . lookup_section($dat["section"]) .')' . '</strong></div>';
	//$output .= '<div style="position:absolute; top:2.5cm; left:12.4cm">' . lookup_yearlevel($dat["year_level"]) . ' ' . lookup_section($dat["section"]) . '</div>';
	...
	for( $i=0; $i<$list->get_numrows(); $i++ ) {
		...
		for( $count=0; $count<$schedule_count; $count++ ) {
			$output .= '<div style="position:absolute; top:' . $y . 'mm; left:144mm">' . mkstr_neat($schedule_ar[$count][0]) . '</div>';
			$output .= '<div style="position:absolute; top:' . $y . 'mm; left:85mm">' . mkstr_neat($schedule_ar[$count][1]) . '</div>';
			$output .= '<div style="position:absolute; top:' . $y . 'mm; left:104mm">' . mkstr_neat($schedule_ar[$count][2]) . '</div>';
			if( $count==0 ) {
				$tmp_y = $y + $height * ($schedule_count-1) / 2;
				$output .= '<div style="position:absolute; top:' . $tmp_y . 'mm; left:5mm">' . $dat["subject_code"] . '</div>';
				$output .= '<div style="position:absolute; top:' . $tmp_y . 'mm; left:58mm">' . mkstr_neat($dat["unit"]) . '</div>';
				list($t,$f,$m,$l) = lookup_teacher_name_array($dat["teacher_id"]);
				$output .= '<div style="position:absolute; top:' . $tmp_y . 'mm; left:164mm">' . mkstr_neat( mkstr_name_fimil($f,$m,$l) ) . '</div>';
			}
			$y += $height;
		}
		$y += 1;
	}
	$output .= '<div style="position:absolute; top:' . $y . 'mm; left:58mm; text-decoration:overline">' . mkstr_neat($total_units) . '</div>';

	/* payment computation display
	$assessment = calc_assessment($sy_id,$student_id);
	...
	*/
	...
	//$output .= '<div style="position:absolute; top:11.7cm; left:0cm; width:' . $size_x . '; text-align:center">' . mkstr_capitalize($dean) . '</div>';
	$output .= '<div style="position:absolute; top:11.1cm; left:1.3in; width:2.5in;">' . mkstr_capitalize($dean) . '</div>';
	...
	$output .= '<div style="position:absolute; top:11.1cm; left:4.4in; width:2.5in;">';
	$output .= '<center>' . mkstr_capitalize($registrar) . '</center>';
	$output .= '</div>';
